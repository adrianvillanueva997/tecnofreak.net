...
function resolveAsset(fromMdocPath: string, ref: string): string | null {
	...
	try {
		const abs = path.resolve(
			path.dirname(fromMdocPath),
			decodeURIComponent(cleaned),
		);
		return abs.startsWith(ASSETS_DIR) &&
			fs.existsSync(abs) &&
...
async function main() {
	...
	console.log(
		`Found ${parsed.length} mdoc posts, ${existingSlugs.size} existing posts in DB`,
	);
	...
	if (categoryFailures.length)
		console.log(
			`category failures (${categoryFailures.length}):\n  ${categoryFailures.join("\n  ")}`,
		);
	console.log(
		`Categories: ${tagTitles.size} unique tags (${stats.categoriesCreated} to create)`,
	);
	...
	for (const p of parsed) {
		...
		try {
			...
			if (p.frontmatter.portada) {
				...
				if (coverPath)
					coverId = await uploadAsset(coverPath, p.frontmatter.title);
				else
					stats.missingAssets.push(
						`${p.slug}: portada ${p.frontmatter.portada}`,
					);
			}
			...
			if (!DRY_RUN) {
				await payload.create({
					...
					data: {
						...
						categories: catIds,
						meta: {
							...(p.frontmatter.descripcion
								? { description: p.frontmatter.descripcion }
								: {}),
							...(coverId ? { image: coverId } : {}),
						},
						publishedAt: p.frontmatter.fecha
							? `${p.frontmatter.fecha}T12:00:00.000Z`
							: undefined,
						_status: "published",
					},
				});
	...
	console.log(
		`media: ${stats.mediaCreated} uploaded, ${stats.mediaReused} reused, ${stats.junkSkipped} junk skipped`,
	);
