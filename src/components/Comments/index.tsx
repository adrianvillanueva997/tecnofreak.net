...
export const CommentsSection: React.FC<{ postId: number }> = async ({
	postId,
}) => {
	...
	return (
		<section
			...
			className="mt-10 border-t border-rule pt-5"
		>
			<h2 className="kicker text-fog">Comentarios ({comments.totalDocs})</h2>
				...
				<ul className="m-0 mt-5 list-none p-0">
					{comments.docs.map((c: Comment) => (
						...
						>
							...
							<time
								className="kicker ml-2 text-fog"
			...
			<p className="kicker mt-3 text-fog">
				Los comentarios pasan revisión antes de publicarse.
			</p>
		</section>
	);
};
