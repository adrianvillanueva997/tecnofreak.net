...
export default async function HomePage() {
  ...
  const [featuredSet, allPosts, categoriesSet] = await Promise.all([
    ...
    payload.find({ collection: 'categories', limit: 0, depth: 0, sort: 'title' }),
  ])
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      {featured ? (
        ...
        <section className="py-16 text-center">
          <h1 className="font-display text-3xl font-bold tracking-tight">Aún no hay artículos</h1>
          <p className="mt-2 text-fog">Publica tu primer artículo desde el panel.</p>
        </section>
      )}
        ...
        <section aria-label="Últimas noticias" className="mt-6">
          <div className="grid gap-x-8 md:grid-cols-[7fr_5fr]">
            ...
            <aside className="mt-10 hidden md:mt-[52px] md:block md:border-l md:border-rule md:pl-8">
              <h2 className="kicker border-b-2 border-teal pb-2 text-fog">Temas</h2>
              ...
              <div className="mt-8 border border-rule bg-paper-2 p-5">
                ...
                <a href="/rss.xml" className="link-more mt-3 inline-block text-sm">
                  Suscribirse
                </a>
              </div>
            </aside>
